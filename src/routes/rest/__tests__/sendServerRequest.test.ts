...
describe('sendServerRequest', () => {
  ...
  it.skip('should handle invalid JSON body by falling back to text/plain', async () => {
  ...
  it.skip('should handle object body by converting it to JSON string', async () => {
    const objectBody = { name: 'Test', value: 123 };
  ...
  it.skip('should calculate response time correctly', async () => {
    const startTime = Date.now();
    vi.setSystemTime(startTime);
    ...
    expect(result.time).toBe(startTime);
    expect(result.duration).toBe(150);
  });

  it.skip("should handle methods that don't accept body correctly", async () => {
    (fetch as Mock).mockResolvedValueOnce({
      ok: true,
      status: 200,
      statusText: 'OK',
