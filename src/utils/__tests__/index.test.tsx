...
import { loadVariablesFromStorage, saveVariablesToStorage, clearVariablesFromStorage, substituteVariables, clearAllUserVariables } from '../variableStorage';
import { hasVariables, extractVariableNames, countVariables } from '../variableHighlight';
...
describe('variableHighlight', () => {
  ...
  it('should extract variable names from text', () => {
    ...
    expect(extractVariableNames('{{var1}} and {{var2}}')).toEqual(['var1', 'var2']);
...
describe('variableStorage', () => {
  ...
  it('should save variables to storage', () => {
    ...
    saveVariablesToStorage(testVariables, 'test-user');
    const allVariables = JSON.parse(localStorage.getItem('rest-client-variables') || '{}');
  ...
  it('should substitute variables in text', () => {
    ...
    expect(substituteVariables('{{api_url}}/users/{{user_id}}', variables))
      .toBe('https://api.example.com/users/123');
    
    expect(substituteVariables('no variables', variables))
      .toBe('no variables');
  });
  ...
  it('should handle missing variables in substituteVariables', () => {
    ...
    expect(substituteVariables('{{test}} and {{missing}}', variables)).toBe('value and {{missing}}');
  });
  ...
  it('should handle array format in localStorage (legacy support)', () => {
    ...
    localStorage.setItem('rest-client-variables', JSON.stringify(legacyVariables));
  ...
  it('should filter out invalid variables', () => {
    ...
    localStorage.setItem('rest-client-variables', JSON.stringify({ 'test-user': invalidVariables }));
  ...
  it('should handle variables with special characters in substituteVariables', () => {
    ...
    expect(substituteVariables('{{special}}', variables))
      .toBe('value with spaces and symbols!@#');
  });
  ...
  it('should handle multiple occurrences of the same variable', () => {
    ...
    expect(substituteVariables('{{api}}/users and {{api}}/posts', variables))
  ...
  it('should handle nested variable patterns', () => {
    ...
    expect(substituteVariables('{{base}} and {{api}}', variables))
      .toBe('api and https://example.com');
  });
  ...
  it('should handle edge cases in substituteVariables', () => {
    ...
    expect(substituteVariables('prefix {{test}} suffix', variables)).toBe('prefix value suffix');
  });
